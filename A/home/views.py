...
from .forms import UpdateForm, CreateForm, CommentCreateForm, CommentReplyForm, PostSearchForm
...
class HomeView(View):
    ...
    def get(self, request):
        post = Post_User.objects.all()
        if request.GET.get('search'):
            post = post.filter(body__contains=request.GET['search'])
        return render(request, 'home/index.html', {'post':post, 'form':self.form_class})
...
class PostDetailView(View):
    ...
    def setup(self, request, *args, **kwargs):
        self.post_instance = get_object_or_404(Post_User, pk=kwargs['post_id'], slug=kwargs['post_slug'])
...
class PostDeleteView(LoginRequiredMixin, View):
    def get(self, request, post_id):
        post = get_object_or_404(Post_User, pk=post_id)
...
class PostUpdateView(LoginRequiredMixin, View):
    ...
    def setup(self, request, *args, **kwargs):
        self.post_instance = get_object_or_404(Post_User, pk=kwargs['post_id'])
        return super().setup(request, *args, **kwargs)
